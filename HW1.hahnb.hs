...
data Tree
   = Node Int Tree Tree   -- ^ Internal nodes (left tree, right tree)
   | Leaf Int             -- ^ Leaf nodes
  deriving (Eq,Show)
...
--the left subtree must contain nodes less than the head
-- the right must contain nodes greater than the head
-- the left subtree should be a BST
-- the right subtree should be a BST
--
isBST :: Tree -> Bool
isBST (Node x l r) =
    ((maxInt l) < x)      -- is the maximum int in the left tree < current root
    && ((minInt r) > x)       -- is the minimum int in the right tree > current root
    && (isBST l) && (isBST r) -- are the left and right trees BSTs
isBST (Leaf _) = True
