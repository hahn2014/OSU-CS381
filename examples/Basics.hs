...
--sum :: [Int] -> Int --Only works with ints
sum :: Num a => [a] -> a --Works with all number classes
sum []    = 0
sum (h:t) = h + sum t
...
--doubleAll' = map (\x -> 2 * x)
doubleAll' = map (2*) --works the same

-- | Reimplement notAll using map.
notAll' :: [Bool] -> [Bool]
--notAll' = map (\b -> not b)
notAll' = map not --works the same
